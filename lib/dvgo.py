...
class DirectVoxGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
                 ...
                 posbase_pe=5, viewbase_pe=4,prune_rate = 1.0,
                 **kwargs):
...
#-----------------
        # init mask for dct matrix
        # self.mask_dct = torch.nn.Parameter(torch.randn(self.density.shape))
    ...
    def _set_grid_resolution(self, num_voxels):
        ...
        self.world_size[0] = (self.world_size[0]//4)*4
        self.world_size[1] = (self.world_size[1]//4)*4
        self.world_size[2] = (self.world_size[2]//5)*5
...
#------------------------------------       
    ...
    def _getDCTMat(self,inLen):
        """ Generate the DCT transformation matrix with speific length.
            Each COLUMN of the returned matrix denotes a basis.
        """

        m = np.arange(inLen).astype('float32')
        # Translate the n-axis to left for 0.5, to make the Fourier bases
        # symmetric about the line n = -0.5.
        n = (m + 0.5).astype('float32')

        m, n = np.meshgrid(m, n)

        # Compute the cosine transformation matrix
        Q = np.cos(np.pi / inLen * m * n)

        # Normalize the transformation matrix to eliminate the scale influence
        Q /= np.sqrt(inLen / 2.)
        Q[:, 0] /= np.sqrt(2)
    ...
    def block_dct(self,x, rate,
        ...
        total = C * H * W
        remain_num = int(rate * total)
        block = (4, 4, 5)
        
        x = re_x.reshape(D, C // block[0], block[0], H // block[1], block[1], W // block[2], block[2])
        
        P = self._getDCTMat(block[0])
        Q = self._getDCTMat(block[1])
        R = self._getDCTMat(block[2])
    
        out = torch.einsum('bi,sabcdef->saicdef', P, x)
...
#    @torch.no_grad()
    def dct3(self,x):
        ...
        re_x = x.resize(D, C, H, W)#.to(torch.device('cpu'))
        #re_x = re_x.numpy()
        total = C * H * W
        
        remain_num = int(self.prune_rate * total)

        P = self._getDCTMat(C)
        Q = self._getDCTMat(H)
        R = self._getDCTMat(W)

        out = torch.einsum('ij,dikl->djkl', P, re_x)
        # print(out.shape)
        out = torch.einsum('jl,dijk->dilk', Q, out)
        # print(out.shape)
        out = torch.einsum('kl,dijk->dijl', R, out)
        
        #-----------------
        #tmp = out.to(torch.device('cpu'))
        #torch.save(tmp, "./color.pt")
...
#                dct_flat = dctx2[i].flatten() #flat()
#                dct_flat[sorted_idx[:-remain_num]]  = 0
        dctx2 = dctx2 * mask
        
        out = torch.einsum('ij,dikl->djkl', P.T, dctx2)
        out = torch.einsum('jl,dijk->dilk', Q.T, out)
        out = torch.einsum('kl,dijk->dijl', R.T, out)
        
       # print(torch.norm(dct_pre - dctx2) / torch.norm(dct_pre))
        out = out.resize(a,D,C,H,W)
        #out = torch.Tensor(out).to(torch.device('cuda'))
        return out
...
#---------------------------
        #if self.mask_cache is not None:            
            density = self.block_dct(self.density,rate = 0.4)
        #else:
        ...
        if self.pre_act_density:
            ...
            alpha[~mask_outbbox] = self.grid_sampler(
                    rays_pts[~mask_outbbox], self.activate_density(density, interval))
        elif self.in_act_density:
            # in-activation
            density = self.grid_sampler(rays_pts[~mask_outbbox], F.softplus(density + self.act_shift))
            alpha[~mask_outbbox] = 1 - torch.exp(-density * interval)
        else:
            # post-activation
            density = self.grid_sampler(rays_pts[~mask_outbbox], density)
...
def get_rays(H, W, K, c2w, inverse_y, flip_x, flip_y, mode='center'):
    i, j = torch.meshgrid(
        torch.linspace(0, W-1, int(W), device=c2w.device),
        torch.linspace(0, H-1, int(H), device=c2w.device))  # pytorch's meshgrid has indexing='ij'
    i = i.t().float()
    j = j.t().float()
    if mode == 'lefttop':
        pass
